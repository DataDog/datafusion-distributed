...
/// Maximum number of rows sampled from the buffer when estimating per-column NDV.
        ...
        let task = SpawnedTask::spawn(async move {
            ...
            if let Some(result) = input_stream.next().now_or_never() {
                if let Some(batch) = result {
                    ...
                    // still yielding synchronously). Buffer it so its rows are not lost. We cannot
                    // measure a meaningful arrival velocity in this case, so as before, assume the
                    // worst.
                    reporter.set_rows_per_second(MAX_ROWS_PER_SECOND);
                    reporter.set_per_col_bytes_per_second(MAX_BYTES_PER_SECOND);
                    buffer.push(batch?);
                }
                reporter.set_per_col_bytes_ready(buffer.per_col_bytes_ready());
                reporter.set_rows_ready(buffer.rows_ready());
                reporter.set_per_col_ndv(buffer.per_col_ndv());
                reporter.set_per_col_null_pct(buffer.per_col_null_pct());
                return Ok(buffer.chain(input_stream).boxed());
            }
            ...
            reporter.set_per_col_bytes_ready(buffer.per_col_bytes_ready());
            reporter.set_per_col_bytes_per_second(bytes_per_second);
            reporter.set_rows_ready(buffer.rows_ready());
            reporter.set_rows_per_second(rows_per_second);
            reporter.set_per_col_ndv(buffer.per_col_ndv());
            reporter.set_per_col_null_pct(buffer.per_col_null_pct());
...
impl LoadInfoDropHandler {
    ...
    fn set_per_col_bytes_per_second(&mut self, total_bytes_per_second: usize) {
        ...
        let per_col: Vec<usize> = if total_ready == 0 {
            ...
            per_col_ready
                .iter()
                .map(|&ready| (ready / total_ready * (total_bytes_per_second as u64)) as usize)
                .collect()
        };
...
impl RecordBatchBuffer {
    ...
    fn per_col_bytes_ready(&self) -> Vec<usize> {
        ...
        for buffer in self.buffer.iter() {
            for (i, col) in buffer.columns().iter().enumerate() {
    ...
    fn per_col_ndv(&self) -> Vec<f32> {
        let total_rows: usize = self.buffer.iter().map(|b| b.num_rows()).sum();
        if total_rows == 0 {

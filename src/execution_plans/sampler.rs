    ...
    fn kick_off(&self, ctx: Arc<TaskContext>) -> Result<oneshot::Receiver<pb::LoadInfo>> {
        ...
        let task = SpawnedTask::spawn(async move {
            ...
            if let Some(result) = input_stream.next().now_or_never() {
                ...
                reporter.set_per_col_bytes_ready(peek.per_col_bytes_ready());
                reporter.set_rows_ready(peek.rows_ready());
                reporter.set_per_col_ndv(peek.per_col_ndv());
                reporter.set_per_col_null_pct(peek.per_col_null_pct());
                return Ok(peek.chain(input_stream).boxed());
            }
